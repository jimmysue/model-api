...
original_name="model_api"
original_urlname="model-api"
original_description="Awesome model_api created by jimmysue" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/model_api "${name}"
...
then
    ...
    echo "then run 'model_api' or 'python -m model_api'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'model_api' or 'python -m model_api' or 'poetry run model_api'"
fi

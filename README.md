...
- 🎯 Entry points to execute your program using `python -m <model_api>` or `$ model_api` with basic CLI argument parsing.
...
---
# model_api Flask Application

Awesome model_api created by jimmysue
...
```bash
git clone https://github.com/jimmysue/model-api model_api
cd model_api
make install
```
...
```bash
model_api create-db   # run once
model_api populate-db  # run once (optional)
model_api add-user -u admin -p 1234  # ads a user
model_api run
```
